...
const ResumoContasBanco = ({
  userId,
  onNavigateToContas
}: ResumoContasBancoProps) => {
  ...
  const fetchBancos = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("bancos").select("*").eq("user_id", userId).eq("ativo", true).order("saldo_atual", {
        ascending: false
      });
      if (!error && data) {
        setBancos(data);
  ...
  return <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            {resumo.variacaoTotal >= 0 ? <TrendingUp className="h-4 w-4 text-income" /> : <TrendingDown className="h-4 w-4 text-expense" />}
          </CardHeader>
          <CardContent className="pb-3 sm:pb-6">
      ...
      {/* Lista de Bancos */}
      {bancos.length > 0 && <div className="my-[15px] py-0">
          ...
          return <Card key={banco.id} className="cursor-pointer hover:shadow-lg transition-shadow" onClick={onNavigateToContas}>
                  <CardContent className="p-3 sm:p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {variacao !== 0 && <div className="flex justify-between items-center">
          ...
          {bancos.length > 6 && <div className="text-center mt-3 sm:mt-4">
